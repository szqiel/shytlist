...
const OPTIONS = {
  ...
  shot_size: ['ECU', 'CU', 'MCU', 'MS', 'MLS', 'LS', 'VLS', 'OTS'],
  lens: ['14mm', '24mm', '35mm', '50mm', '85mm', '100mm', 'Zoom'],
  movement: ['Static', 'Pan', 'Tilt', 'Track', 'Dolly', 'Handheld', 'Gimbal', 'Crane', 'Drone'],
...
export default function ShotlistEditor() {
  ...
  const saveProjectInfo = async () => {
    if (!project || !tempName || !tempDirector) return;
    try {
      setSaveStatus('saving');
      const { error } = await supabase
        .from('projects')
        .update({ title: tempName, director: tempDirector })
        .eq('id', project.id);

      if (error) throw error;
      
      setProject({ ...project, title: tempName, director: tempDirector });
  ...
  const exportPDF = () => {
    ...
    // Aesthetic: Dark Theme PDF Header
    ...
    autoTable(doc, {
      startY: 45,
      head: [['#', 'SCENE', 'SIZE', 'LENS', 'MVMT', 'ANGLE', 'FRAME', 'DESCRIPTION']],
      body: body,
      theme: 'grid',
      headStyles: { fillColor: [23, 23, 23], textColor: [55, 202, 255], fontSize: 8, fontStyle: 'bold' },
  ...
  return (
    <div className="flex flex-col h-full bg-bg -mx-6 md:-mx-12 -my-8 overflow-hidden min-h-[calc(100vh-64px)]">
      <div className="flex h-full flex-1 overflow-hidden">
        ...
        <aside className="w-80 bg-zinc-950 border-r border-white/5 p-8 hidden lg:flex flex-col gap-10">
          ...
          <div className="space-y-8">
            <div className="p-6 bg-zinc-900/50 rounded-2xl border border-white/5 space-y-4">
              ...
              <div className="flex justify-between items-center text-sm">
                ...
                <span className="text-brand-cyan font-semibold text-[10px] uppercase tracking-wider">In Production</span>
              </div>
            </div>
          ...
          <div className="mt-auto">
            <div className="flex items-start gap-3 text-zinc-600 bg-zinc-900/30 p-5 rounded-2xl border border-white/5">
               <Film className="w-5 h-5 shrink-0 text-zinc-700 mt-0.5" />
               <p className="text-xs leading-relaxed font-medium">Build for the masterpiece. Every shot matters.</p>
            </div>
          </div>
        </aside>
        ...
        <section className="flex-1 flex flex-col bg-bg overflow-hidden">
          ...
          <div className="px-8 py-6 bg-nav border-b border-white/5">
            <form onSubmit={addShot} className="grid grid-cols-12 gap-3 items-end">
              ...
              <div className="col-span-5">
                ...
                <input 
                  required
                  type="text" 
                  placeholder="Cinematic intent..."
                  className="input-field focus:ring-1 focus:ring-brand-yellow/20" 
              ...
              <table className="w-full text-left border-collapse min-w-[1100px]">
                <thead className="bg-zinc-900/50 border-b border-white/5">
                  <tr className="text-[10px] uppercase tracking-widest text-zinc-500 font-semibold">
                    ...
                    <th className="px-6 py-5 w-16 text-brand-cyan">#</th>
                    <th className="px-6 py-5 w-20">Scene</th>
                    <th className="px-6 py-5 w-24">Size</th>
                    <th className="px-6 py-5 w-32">Lens</th>
                    <th className="px-6 py-5 w-32">Movement</th>
                    <th className="px-6 py-5 w-32">Angle</th>
                    <th className="px-6 py-5">Description</th>
                    <th className="px-6 py-5 w-20 text-right pr-10"></th>
                  </tr>
                </thead>
                <DragDropContext onDragEnd={handleDragEnd}>
                  <Droppable droppableId="shotlist">
                    {(provided) => (
                      <tbody {...provided.droppableProps} ref={provided.innerRef} className="divide-y divide-white/5">
                        {shots.length === 0 ? (
                          <tr>
                              ...
                              {(provided: any, snapshot: any) => (
                                ...
                                >
                                  <td className="px-6 py-5">
                                    <div {...provided.dragHandleProps} className="text-zinc-700 group-hover:text-zinc-500 transition-colors cursor-grab active:cursor-grabbing">
                                      <GripVertical className="w-4 h-4" />
                                    </div>
                                  </td>
                                  <td className="px-6 py-5 text-zinc-300 text-sm font-semibold tracking-tighter">{shot.shot_no}</td>
                                  <td className="px-6 py-5 text-zinc-500 text-sm font-medium">Sc. {shot.scene_no}</td>
                                  <td className="px-6 py-5">
                                    <span className="bg-zinc-800/80 text-[10px] px-2.5 py-1 rounded-md border border-white/5 font-semibold text-zinc-300">
                                      {shot.shot_size}
                                    </span>
                                  </td>
                                  <td className="px-6 py-5 text-sm font-medium text-zinc-400">{shot.lens}</td>
                                  <td className="px-6 py-5 text-sm font-medium text-zinc-500">{shot.movement}</td>
                                  <td className="px-6 py-5 text-sm font-medium text-zinc-500">{shot.angle}</td>
                                  <td className="px-6 py-5">
